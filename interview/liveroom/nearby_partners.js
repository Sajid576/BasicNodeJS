...
const london_lon= -0.141099;
//Haversine formula to calculate distance between two coordinates
getDistance=function(center_lat,center_lon,lat,lon)
{
      
      var R = 6371; // Radius of the earth in km
      var dLat = deg2rad(center_lat - lat); // deg2rad below
      var dLon = deg2rad(center_lon - lon);
      
      var a = Math.sin(dLat / 2) * Math.sin(dLat / 2) +
              Math.cos(deg2rad(lat)) * Math.cos(deg2rad(center_lat)) *
...
function findNearbyPartners(mylist)
{
    nearbyPartners=[]

    mylist.forEach(function(partner) {
     
        var officeList=partner.offices;
        ...
        if(officeList)
        {
          ...
          if(officeList.length>0)
          {
              ...
              officeList.forEach(function(office){
                var off_lat=Number( office.coordinates.split(",")[0] );
                var off_lon=Number(office.coordinates.split(",")[1]);
                 
                let dist=getDistance(london_lat,london_lon,off_lat,off_lon);
                if(dist<=100)
                {
                    nearbyPartners.push({
...
// sort the list according to the company name
  nearbyPartners.sort((a, b) => (a.company_name > b.company_name) ? 1 : -1)

  return nearbyPartners;
}

var nearbyPartners=findNearbyPartners(partnerList);
